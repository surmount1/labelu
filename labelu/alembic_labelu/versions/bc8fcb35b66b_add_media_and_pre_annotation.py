    ...
    try:
        ...
        with context.begin_transaction():
            ...
            for attachment in attachments:
                ...
                if filename:
                    session.execute(
                        text(f"UPDATE task_attachment SET filename='{filename}', url='{url}' WHERE id={attachment_id}")
                    )
            ...
            if column_exist_in_table("task_sample", "task_attachment_ids"):
                for task_item in task_items:
                    ...
                    task_samples = session.execute(
                        text(f"SELECT id, task_attachment_ids FROM task_sample WHERE task_id={task_id}")
                    )
                    ...
                    for task_sample in task_samples:
                        ...
                        attachment = session.execute(
                            text(f"SELECT id, path FROM task_attachment WHERE id={file_id}")
                        )
                        ...
                        if attachment_path:
                            # Update the task_sample table
                            session.execute(
                                text(f"UPDATE task_sample SET file_id={file_id} WHERE id={task_sample_id}")
                            )
